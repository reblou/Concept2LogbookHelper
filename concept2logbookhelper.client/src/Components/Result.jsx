...
function Result({ date, type_pretty, type, distance, time, pace, spm, calories, avg_hr, link }) {
  ...
  return (
      <tr>
          ...
          <td><div className={'workout_type'} style={{background: "var(--typeColour-" + type + ")", minWidth: "fit-content"  }} onClick={() => filterResultsCallback(r => r.pretty_workout_type, v => v===type_pretty, "Type", type_pretty) }>{type_pretty}</div></td>
          <td>{time}</td>
          <td>{distance}m</td>
          <td>{pace}</td>
          <td>{spm}</td>
